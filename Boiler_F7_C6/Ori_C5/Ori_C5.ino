...
#define WIFI_PASSWORD "F07#APDEVICE@"
  // Line config
//#define LINE_TOKEN "Jf1rnCnuUItppNY207jhyDOT9gjX4QwzXoZty7j0fcc" // C5
...
//#define high_temp2 D0
//#define high_temp3 D3
#define low_temp1 D2
//#define low_temp2 D4
//#define low_temp3 D5
#define run_mode D7
//#define high_furnace D7
//#define low_furnace D8
...
String message_tempLOW = " Supply Temp LOW [Less than 248 C]";
...
String message_hello = "startting system";
int cout = 0;
int cout_temp = 1;
int check=1;
...
//  pinMode(high_temp2, INPUT_PULLUP);
//  pinMode(high_temp3, INPUT_PULLUP);
  pinMode(low_temp1, INPUT_PULLUP);
//  pinMode(low_temp1, INPUT_PULLUP);
//  pinMode(low_temp1, INPUT_PULLUP);
      pinMode(run_mode, INPUT_PULLUP);
//  pinMode(high_furnace,INPUT_PULLUP);
//  pinMode(low_furnace,INPUT_PULLUP);
  Serial.begin(115200);
...
void loop()
{
  ...
  if(digitalRead(run_mode) == LOW){
    ...
    while(digitalRead(high_temp1) == HIGH && digitalRead(low_temp1) == HIGH){ 
      check=1;
      Serial.println(check);
      if(check==1){
        Line_Notify(message_tempINSPEC);
      }
      if (check>900)  //-->900= Alarm to Time 15min
      {
